...
from timing_utilities.timers import time_fn
from os import path
import pandas as pd
...
@time_fn
def train_model_data_drift() -> None:
    ...
    log_.info('Accuracy and auc on training and test datasets=%s', str(dict_acc_auc))
...
def main():
    ...
    if path.exists('../data/input/train.csv') & path.exists('../data/input/test_reference.csv'):
        log_.info('Pre-processed data exists, proceeding with data drift analysis and model training...')
        train_model_data_drift()
        log_.info('Model has been trained and data drift report has been generated...')
    else:
        log_.info('Pre-processed data does not exist. Starting raw data processing...')
        process_raw_data()
        log_.info('Raw data has been processed and is ready for model training...')
        train_model_data_drift()
        log_.info('Model has been trained and data drift report has been generated...')
